...
async def show_stats_callback(
    ...
    try:
        ...
        from utils.keyboards import stats_keyboard
...
async def detailed_stats_callback(
    ...
    # رسم بياني نصي
    chart = _build_text_chart(type_stats)
...
async def storage_stats_callback(
    ...
    for key, name in media_types.items():
        ...
        if size > 0:
            ...
            bar = _build_mini_bar(size, total_size)
            msg += (
                f"{name}\n"
...
async def compare_archives_callback(
        update: Update,
        context: ContextTypes.DEFAULT_TYPE):
    ...
    for i, archive in enumerate(sorted_archives[:5], 1):
...
def _build_text_chart(data: dict) -> str:
    """رسم بياني نصي"""
    if not data:
    ...
    for key, value in sorted(
        ...
        bar        = _build_mini_bar(value, total)
        result    += (
            f"{icon} {bar} "
